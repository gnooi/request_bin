...
async function purgeRequestsOlderThan(cutoffDate) {
  ...
  while (true) {
    ...
    const { rows } = await pool.query(deleteQuery, [cutoffDate, BATCH_SIZE]);
    ...
    for (const [binId, count] of Object.entries(countsByBin)) {
      await pool.query(updateCountsQuery, [count, binId]);
    }
...
async function purgeOldRequests() {
  ...
  console.log(
    `Purged ${deleted} request(s) older than ${cutoff.toISOString()}`,
  );
  return deleted;
}
